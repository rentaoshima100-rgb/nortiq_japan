...
IT導入補助金については、中小企業庁が[公式サイト](https://it-shien.smrj.go.jp/)で制度詳細を随時更新しています。単なる情報発信目的のサイト制作は補助対象外とされており、受発注管理・予約機能・EC機能など業務プロセスと連動する機能を含む場合に限り対象となる可能性があります。対象要件の詳細は次のセクションで解説します。
...
当社は、事業計画の整理から制作・納品までを同じチームで一貫して対応します。ただし、当社はIT導入支援事業者に未登録のため、IT導入補助金（2026年度からは「デジタル化・AI導入補助金」）を使った申請支援・ツール提供は現状できません。御社のリニューアルがどの制度の対象になり得るかは、無料相談で確認できます。当社の前提は[補助金活用ページ（/subsidy）](/subsidy)にまとめています。
...
当社代表はAIエンジニアであり、RAGやFine-tuningを自ら実装する実務者です。「コンサルが書いた計画書」ではなく、「実装者が技術的実現性を担保した計画書」を作成できることが最大の差別化です。

### 対応できる制作規模と費用の目安

当社のWeb制作は30万円〜（税別）で対応しています。小規模事業者持続化補助金など、御社が申請する制度の対象費用に合わせた見積もりを作成するため、補助対象外の費用を誤って申請に含めるリスクを最小化できます。

AIチャットボット機能をリニューアルと同時に追加したい場合も、初期費用10万円〜、月額1万円〜（税別）の構築プランを組み合わせて一括対応が可能です。IT導入補助金ではCMS・チャットボットなどの複数機能を同一申請にまとめられるケースがあるため、リニューアル単体よりも補助対象の費用総額を大きくできる場合があります（同補助金の申請には登録済みのIT導入支援事業者の関与が必要で、未登録の当社経由では申請できません）。
