...
// meta に書く金額は content-data.jsx の NORTIQ_PRICING から作る (app.jsx は最後に読み込まれるので
// モジュール直下で参照できる)。ここは meta description・og:description・Service JSON-LD の
// description の共通の出典なので、手書きにすると検索結果とページ本文の金額がずれる。
// 業種ソリューション5本は同じ型の文を末尾に足す。以前は不動産だけに下限の金額があり、
// 上限も月額も無く、他の4業種には金額が無かった。
const solutionMeta = (key, body) =>
  `${body}目安は初期${priceInitPlusMonthly(NORTIQ_PRICING.solutions[key])}${priceTax()}。`;
const SEO_DESC = {
  top: DEFAULT_DESC,
  web: 'オリジナルデザインのホームページ制作。SEO設計・高速表示（Core Web Vitals対応）・CMS・アクセス解析まで標準対応。京都から全国のBtoB・店舗ビジネスを支援します。',
  // 「◯万円台」は上限があるように読めるので使わない。料金表と同じ「初期費用」＋開始価格で書く。
  chatbot: `自社サイトのFAQ・問い合わせ対応を自動化するAIチャットボット導入。WordPress連携・LINE連携・自社データ学習に対応。初期費用${priceFrom(NORTIQ_PRICING.chatbot.plans[0].min)}${priceTax()}、京都のNortiq Labsが構築します。`,
  ...
  // /subsidy 本文の「正直な前提」(content-data.jsx の SUBSIDY_CONTENT.honest) に合わせる。
  // 以前は補助額を確認できると書いていたが、当社は IT導入支援事業者に未登録で、
  // 単純なホームページ制作は旧IT導入補助金の対象外。補助される前提で予算を組まれないようにする。
  subsidy: 'ホームページ制作・AIツール導入で補助金を検討する方のご相談窓口。当社はIT導入支援事業者に未登録で、単純なホームページ制作は旧IT導入補助金の原則対象外です。持続化補助金など使える可能性のある制度を、初回無料でご案内します。',
  ...
  'solution-clinic': solutionMeta('clinic', 'クリニックの集患をWeb×DXで支援。医療広告ガイドライン準拠のサイト制作、Googleビジネスプロフィール（MEO）整備、AIチャットボット・Web予約連携までパッケージで提供します。'),
  'solution-realty': solutionMeta('realty', '売買・賃貸・売却査定・投資物件それぞれの導線を最適化したWebサイトと、物件管理データ連動の集客・追客システム。必要な機能だけ選んで導入できます。'),
  'solution-build': solutionMeta('build', '施工品質が伝わるブランドサイトと、見積・案件管理のDXをセットで。大規模修繕・住宅・リフォーム各分野の制作実績に基づく、建築業特化のWebパッケージです。'),
  'solution-hr': solutionMeta('hr', '求人検索・スカウト・応募管理を備えたマッチングサイト構築と、SEO・広告での集客支援をワンストップで。人材紹介・派遣業のWeb基盤を短期間で立ち上げます。'),
  'solution-retail': solutionMeta('retail', 'Shopifyを軸に店舗とECの在庫・顧客・販促を統合するOMOパッケージ。POS連携、店舗受け取り、会員統合まで。小売・ECの「バラバラ運用」を一つにします。'),
