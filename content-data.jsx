...
const GUIDEBOOK_CONTENT = {
  ...
  faqs: [
    ...
    { q: "どんな内容ですか?", a: "制作の進め方・実績・料金プラン・制作の流れを全9ページにまとめています。" },
  ],
};
...
// -------------------- 業種別のシステム提案テンプレート --------------------
// 方針:
//  - 各案は「想定シナリオ」であることを明示する。実在の納品実績は
//    TAKETORA の AI在庫登録システムのみで、それは works に実績として載せている
//  - 費用・期間はレンジの目安。要件で動くことを毎回明記する
//  - 業界の状況として書く数値は出典を持つものだけにし、法令は官公庁の一次情報を引く
const SYSTEM_TEMPLATES = {
  'solution-clinic': {
    lede: "クリニックのシステム化は、入れること自体より「現場が実際に使えるか」で結果が分かれます。2026年度の診療報酬改定でも、医療DX関連の評価が「導入しているか」から「実際に活用され、利用率の目標を満たしているか」という実績ベースへ移りました。以下は、当社がご提案できる構成の型です。",
    items: [
    ...
    law: { t: "2026年度診療報酬改定の方向", d: "医療DX関連の加算が整理・統合され、システムを導入しているだけでなく実際の利用率が問われる形になりました。導入の順番と、使われる導線の設計が以前より重要になっています。",
      src: { label: "参照: 厚生労働省 診療報酬改定", url: "https://www.mhlw.go.jp/stf/seisakunitsuite/bunya/0000188411_00051.html" } },
  },
  'solution-realty': {
    lede: "不動産は反響の一次対応と、契約・管理の事務が同時に重い業種です。反響への応答が追いつかないまま件数だけ増えると、広告費が成果に変わりません。以下は、そのどこを機械に渡すかの型です。",
    items: [
  ...
  'solution-build': {
    lede: "建築・工務店で最初に効くのは、案件ごとの採算が締めてから分かる状態を、進行中に分かる状態へ変えることです。従業員30名以下の会社では、工事別の原価が見えないまま期末を迎える例が多いといわれます。",
    items: [
    ...
    law: { t: "電子取引データの保存", d: "電子でやり取りした取引情報は、電子のまま保存する必要があります。検索要件・改ざん防止措置の要否は事業規模等で変わるため、設計前に条件を確認します。",
      src: { label: "出典: 国税庁 電子帳簿保存法関係", url: "https://www.nta.go.jp/law/joho-zeikaishaku/sonota/jirei/" } },
  },
  'solution-hr': {
    lede: "人材業のシステム化は、求職者・求人・稼働の三つの台帳が別々に育ってしまうところが起点です。派遣では派遣元・派遣先・スタッフの三者が絡むため、勤怠と契約の事務が特に重くなります。",
    items: [
  ...
  'solution-retail': {
    lede: "小売・EC で最も痛いのは、店舗とネットで在庫が合わないことです。売れるはずのものが売れず、無いものが売れてしまいます。ここを直すと、他の改善の効きも変わります。",
    items: [
...
// 業種別のシステム提案。実績の主張ではなく「こういう場合にこう組めます」の型として出す
function SystemTemplates({ pageId }) {
  ...
  return (
    <div className="container" style={{ maxWidth: 980, marginTop: 64 }}>
      ...
      <div className="sys-grid fadein">
        {d.items.map((it, i) => (
          <article key={i} className="sys-card">
            <div className="sys-card-head">
              <span className="sys-num">{String(i + 1).padStart(2, '0')}</span>
              <h3>{it.t}</h3>
            </div>
            <p className="sys-scenario"><span>想定</span>{it.scenario}</p>
            <p className="sys-does">{it.does}</p>
            <div className="sys-meta">
              <span><em>費用の目安</em>{it.price}</span>
              <span><em>期間の目安</em>{it.weeks}</span>
            </div>
            <p className="sys-note">{it.note}</p>
          </article>
        ))}
      </div>
      {d.law && (
        <div className="sys-law fadein">
          <h3>{d.law.t}</h3>
          <p>{d.law.d}</p>
          <a href={d.law.src.url} target="_blank" rel="noopener noreferrer">{d.law.src.label}</a>
        </div>
      )}
    </div>
  );
