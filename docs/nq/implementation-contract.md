...
- **`<NqSlot slot="slot-mid|slot-end|slot-next" defaultBlock={id} onNavigate={fn} />`**
  ...
  - マークアップは設計書7章のとおり（`<aside class="nq-slot" data-slot aria-label="おすすめのページ">` の中に
  ...
  - `nq_shown` は IntersectionObserver（threshold 0.5）でページ表示ごとに1回。`nq_click` はリンク押下時。
    デフォルト表示でも送る（`decision_id:null, is_default:true`）。
...
- `StickyCTA`（components.jsx）: 見た目は変えない。(a) `NQ.get('slot-bar')` が `ct-*` の strong を返したら文言とボタンを差し替え
  ...
  (c) `role="region"` `aria-label="ご相談の案内"`、閉じるボタンの当たり判定 44×44px。(d) `nq_dismiss`。SP には何も出さない。
...
- 処理の順: メソッド／ボディ上限 8KB ／`NQ_ENABLED!=='1'` なら即デフォルト → Origin 検証（nortiqlab.com・*.vercel.app・localhost）→
...
| `guard.js` | `checkOrigin(req)` `isBot(ua)` `validSessionId(s)` `isHoldout(sessionId, rate)` |
...
| `recommend.js` | `recommend({ answers, candidates, slots, currentUrl, revisit, model, policy, rng })` → `{ picks: { [slot]: { block_id, propensity } }, candidates: [...ログ用], policy, explored }`。純関数（rng を注入できる） |
| `model.js` | `async loadModel()` → 最新の `nq_model` 行（mean / variance / aux）を Supabase から読み、モジュールスコープに10分キャッシュ。未設定・失敗・0行なら `priorModel()`。応答を遅らせない（300ms 上限） |
| `decide.js` | `async decide(state, questions, opts)` → `{ provider, model, answers, latency_ms }`。`NQ_MODEL_PROVIDER` = `stub`（既定）/ `jev`。answers は正規化形（下記） |
...
- **候補**: 6.2 の candidates のうち、関連度 ≥ `rel_floor`（0.35）のカード。最大の関連度 < `rel_gate`（0.55）ならデフォルト（探索もしない）。
...
- **方策**（env `NQ_POLICY`）:
  - `prior`（既定。フェーズ1〜2）: 事前分布の平均（学習済みモデルがあっても使わない）で期待値を計算し最大を選ぶ。一様探索5%だけ。
...
- **ログ**: `candidates` = 候補ごとの `{ block_id, rel, features, score（事前/事後平均での期待値）, propensity }`。
...
`{ session_id, decision_id|null, type: "shown|click|engaged|dismiss|goal", slot, block_id, variant, page_url, goal, read }`（`read` は engaged のときの読み方 deep / skim）。
...
- `supabase/nq_schema.sql`: nq_decisions（設計書10章の列。candidates / policy を含む）、nq_events（＋ variant, session_id, page_url, goal, read。decision_id は null 可）、
