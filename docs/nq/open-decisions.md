...
### D10. 行6（強い CTA）がほとんど発動しない: cta_ok と検討度のしきい値【フェーズ1の前】
- **現状**: **しきい値は変えていない**（`data/nq-rules.json` の `cta_ok: 0.7`、`stage_cta: 2`）。1回目の評価では cta_ok が1件も 0.7 を超えなかった
...
- **決めてほしいこと**: 検討度の基準の文とページ種別の名前を先に直して測り直すか、`stage_cta` を先に 1.5 へ下げるか
  （下げるだけなら、いまの回答で 当たり7・外れ2。外れは s10 と s58）。PC でしか出ない表示（C2）なので、急ぐかどうかも含めて。
- **変える場所**: `data/nq-labels.json`（`stage` の基準の文、`page_type_labels`）、`data/nq-rules.json`（`thresholds.stage_cta` / `cta_ok`）。
  変えたら同じ60件で確かめ、しきい値を変えた場合はここに前後の数字を記録する。数字の出どころは `docs/nq/eval-2026-09-21.md` の「2回目」。

### D11. 着地ページの need タグを Jev に渡し続けるか、関連度の下限 0.35 でよいか【フェーズ1の前】
- **現状**: `api/_lib/state.js` は、catalog の need タグ（記事は `data/catalog-articles.json` の `category_defaults`。技術 → 業務システム・DX、
...
- **決めてほしいこと**: need タグを (a) 渡すのをやめる、(b) 技術系カテゴリの既定のタグだけ空にする、(c) 履歴のページにも付ける、のどれにするか。
  `rel_floor` を 0.45 に上げるか（カード全体の 当たり/外れ/見逃し は 51/44/4 → 44/26/11。1枚目の差し替えは `rel_gate` 0.55 で決まるので、効くのは候補の並びと2枚目）。
- **変える場所**: `api/_lib/state.js`、`data/catalog-articles.json` の `category_defaults`、`data/nq-rules.json` の `thresholds.rel_floor`
  （変えたら E4 のとおり `supabase/nq_report.sql` の直書きも確認）。変えたら同じ60件で確かめる。
...
- **現状**: `eval/sessions.json` の60セッションは LLM が作った下書きのまま。**人によるラベルの見直しは、まだ行われていない。** 迷うものは `expected.alt` に許容ラベルを並べてある。
...
- **決めてほしいこと**:
  1. ラベルを見直す人と時期。フェーズ1（シャドーモード）へ進む判断の前提になる。
  2. 完了条件「訪問者タイプの一致が8割以上」をどの数字で見るか。完全一致は、第1ラベルと許容ラベルの順番だけで約7ポイント動く。
     監査の案は「許容ラベルも正解とする数字を主にし、確信あり（0.6 以上）での正答と、ルールの分かれ目の一致を併せて見る」。
  3. 正解カード（`relevant_cards`）の定義。「状態に直接の手がかりがあるカードだけ」にするか、「次の一歩として見せたいカード」も含めるか。
  4. 事業者でない訪問者のニーズの書き方（監査の案は、第1は other のまま、じっくり読んだ着地記事の need タグを許容ラベルに入れる形で全件を揃える）。
  5. しきい値やラベルを変える前後の比較に使う「基準の結果」を1つだけコミットして残すか
     （キーも個人の情報も含まない。残すなら `git add -f` か、`eval/baseline/` のような別の置き場にする）。
- **変える場所**: `eval/sessions.json`（監査文書の6章から採るものを写し、`node eval/run.js --check` → `node eval/run.js --provider jev`）。
  完了条件の数字を決めたら `README.md` 5章。基準の結果を残すなら `.gitignore`。`eval/run.js` の「none の一致」が許容ラベルを見ていない点も、見直しと一緒に直すとよい。
